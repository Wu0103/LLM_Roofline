...
N = abs(STOP - START + 1)
Ele_size = 2
def count(info,Data,batch,maxLength,Column):
    ...
    # Latency=[batch,maxLength,Atten_Latency,FFN_Latency]
    # csv_file_path = "./files/ColLatency.csv"

    Computation_latency = (Atten_Flops+FFN_Flops)/(Perf*1024*1024*1024/TP) + (Atten_Flops_+FFN_Flops_)/(GPUPerf*1024*1024*1024)
    Communication_latency = Ele_size*(Atten_Read+Atten_Write+FFN_Read+FFN_Write)/(SysBW*1024*1024*1024/TP)
    Latency=[batch,maxLength,Communication_latency,Computation_latency]
    if Column == True:
        csv_file_path = "./files/ColLatency.csv"
    else:
        csv_file_path = "./files/RowLatency.csv"
